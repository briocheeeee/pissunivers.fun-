...
class PixelPainterControls {
  ...
  static placeSinglePixel(state, renderer, x, y, selectedColor) {
    const { canvasSize, clrIgnore, palette } = state.canvas;
    const maxCoords = canvasSize / 2;
    if (x < -maxCoords || x >= maxCoords || y < -maxCoords || y >= maxCoords) {
      return;
    }
    const curColor = renderer.getColorIndexOfPixel(x, y);
    ...
    pixelTransferController.tryPlacePixel(i, j, offset, selectedColor, curColor);
  }
  ...
  static placePixel(store, renderer, cell, colorIndex = null) {
    ...
    if (cx > 5275 && cy > -8713 && cx < 5398 && cy < -8614) {
    ...
    const needsPerPixelColor = brushSize > 1
      && (state.gui.holdPaint || pencilMode !== PENCIL_MODE.COLOR);
    ...
    for (let dx = -half; dx <= half; dx += 1) {
      for (let dy = -half; dy <= half; dy += 1) {
        ...
        if (needsPerPixelColor) {
          color = PixelPainterControls.getWantedColor(state, renderer, [px, py]);
          if (color === null) {
            continue;
          }
        }
        PixelPainterControls.placeSinglePixel(state, renderer, px, py, color);
      }
    }
  }
