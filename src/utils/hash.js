...
export function generateHash(password) {
  return bcrypt.hashSync(password, bcrypt.genSaltSync(BCRYPT_COST_FACTOR), null);
}
...
export function needsRehash(hash) {
  if (!hash || hash === 'hacked') return false;
  const rounds = bcrypt.getRounds(hash);
  return rounds < BCRYPT_COST_FACTOR;
}

export async function rehashPassword(password) {
