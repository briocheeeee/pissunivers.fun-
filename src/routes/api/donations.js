<<<<<<< C:/Users/brioc/Documents/GitHub/pixuniverse/src/routes/api/donations.js
...
import { initDonationService, getDonationService } from '../../core/donations/DonationService.js';
import { User } from '../../data/sql/index.js';
import { getChatBadgeSlugs } from '../../data/sql/Badge.js';
...
router.get('/badges/available', ensureLoggedIn, attachDonationPermissions, async (req, res) => {
  const earnedBadgeSlugs = await getChatBadgeSlugs(req.user.id);

  const tierBadges = [];
  if (req.isVip) tierBadges.push('vip');
  if (req.isPremium) tierBadges.push('premium');

  const allAvailable = [...new Set([...earnedBadgeSlugs, ...tierBadges])];

  const badgeDetails = allAvailable.map((slug) => {
    const config = badgesConfig.badges.find(
    ...
    return {
      ...
      file: config?.file || `${slug}.gif`,
      thumb: config?.thumb || `${slug}.webp`,
    };
  });
...
router.post('/badges/set', ensureLoggedIn, attachDonationPermissions, async (req, res) => {
  ...
  const allowedBadges = [...new Set([...earnedBadgeSlugs, ...tierBadges])];
  const validBadges = badges.filter((b) => allowedBadges.includes(b));
...
router.get('/badges/all', (req, res) => {
  const badges = badgesConfig.badges.map((b) => ({
...
export default router;
>>>>>>> C:/Users/brioc/.windsurf/worktrees/pixuniverse/pixuniverse-26cf27c6/src/routes/api/donations.js
