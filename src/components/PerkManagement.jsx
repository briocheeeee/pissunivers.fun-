<<<<<<< C:/Users/brioc/Documents/GitHub/pixuniverse/src/components/PerkManagement.jsx
...
import { useSelector } from 'react-redux';
import { t } from 'ttag';
import { FaPalette, FaCheck, FaCrown, FaStar, FaMedal } from 'react-icons/fa';
...
const PerkManagement = () => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const [nicknameRes, profileRes, badgesCurrentRes, badgesAvailableRes] = await Promise.all([
          fetch('/api/donations/nickname/options'),
          fetch('/api/donations/profile/options'),
          fetch('/api/donations/badges/current'),
          fetch('/api/donations/badges/available'),
        ]);
        ...
        if (badgesCurrentRes.ok) {
          const badgesData = await badgesCurrentRes.json();
          setSelectedBadges(badgesData.badges || []);
        }

        if (badgesAvailableRes.ok) {
          const availableData = await badgesAvailableRes.json();
  ...
  const toggleBadge = (badgeSlug) => {
    setSelectedBadges((prev) => {
      if (prev.includes(badgeSlug)) {
        return prev.filter((b) => b !== badgeSlug);
      }
      return [...prev, badgeSlug];
    });
  };

  const getBadgeIcon = (slug) => {
    if (slug === 'vip') return <FaStar style={{ color: '#f59e0b' }} />;
    if (slug === 'premium') return <FaCrown style={{ color: '#8b5cf6' }} />;
    return <FaMedal style={{ color: '#3b82f6' }} />;
  };
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        {availableBadges.length === 0 ? (
          <p style={{ fontSize: 13, opacity: 0.7, margin: 0 }}>
            {t`No badges available yet. Earn badges by completing achievements!`}
          </p>
        ) : (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {availableBadges.map((badge) => (
              <button
                ...
                style={{
                  padding: '8px 12px',
                  borderRadius: 6,
                  border: selectedBadges.includes(badge.slug) ? '2px solid #22c55e' : '1px solid rgba(0,0,0,0.2)',
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                  gap: 8,
                  backgroundColor: selectedBadges.includes(badge.slug) ? '#f0fdf4' : 'white',
              ...
              >
                ...
                {selectedBadges.includes(badge.slug) && <FaCheck style={{ color: '#22c55e' }} />}
              </button>
            ))}
          </div>
        )}
...
export default PerkManagement;
>>>>>>> C:/Users/brioc/.windsurf/worktrees/pixuniverse/pixuniverse-26cf27c6/src/components/PerkManagement.jsx
