 ...
 */
import { USE_XREALIP, TIMEBLOCK_IPS, PROXY_RISK_THRESHOLD } from '../core/config.js';
import {
  sanitizeIPString, ipToHex, getHostFromRequest,
} from '../utils/intel/ip.js';
import { getProxyRateLimiter } from '../utils/intel/ProxyRateLimiter.js';
...
const rateLimiter = getProxyRateLimiter();
...
export class IP {
  ...
  async getAllowance(refresh = false) {
    ...
    if (!this.#allowance || refresh
      ...
      this.riskScore = allowance.riskScore || 0;
      this.isProxy = !allowance.isWhitelisted && (
        allowance.isProxy || this.riskScore >= PROXY_RISK_THRESHOLD
      );

      if (this.riskScore > 0) {
        rateLimiter.recordRequest(ipString, this.riskScore);
      }
      if (rateLimiter.isBlocked(ipString)) {
        this.isProxy = true;
