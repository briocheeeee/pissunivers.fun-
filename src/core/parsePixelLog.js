...
import { getUserInfosToIds } from '../data/sql/User.js';
import {
  getIIDsOfIPs,
  getIPInfos,
  getIPofIID,
 ...
 *         string if error
 */
export async function getIIDFlags(
  iid,
  time,
) {
  ...
  const ipsFound = new Set();
  const uidsFound = new Set();
  try {
    await parseFile((parts) => {
      const [tsStr, ipString, uidStr] = parts;
      const ts = parseInt(tsStr, 10);
      if (ts >= time && ipString === filterIP) {
        ipsFound.add(ipString);
  ...
  if (ipsFound.size === 0) {
    return 'No pixels found for this IID during cache period';
  }

  const ipArray = Array.from(ipsFound);
  const [ipInfos, uid2Info] = await Promise.all([
    getIPInfos(ipArray, null),
    getUserInfosToIds([...uidsFound]),
  ]);

  const flagSet = new Set();
  ipInfos.forEach((ipInfo) => {
    if (ipInfo?.country && ipInfo.country !== 'xx') {
      flagSet.add(ipInfo.country.toLowerCase());
    }
  });

  const users = [];
  uidsFound.forEach((uid) => {
    const info = uid2Info.get(uid);
    users.push({
      id: uid,
      name: info?.name || null,
  ...
  const columns = ['rid', 'flag'];
  const types = ['number', 'flag'];
  const rows = [];
  ...
  return {
    ...
    rows,
    users,
    info: `Found ${flags.length} country/countries for this IID during cache period`,
  };
}
