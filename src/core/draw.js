<<<<<<< C:/Users/brioc/Documents/GitHub/pixuniverse/src/core/draw.js
/*
...
import { getBehaviorAnalyzer } from '../utils/intel/BehaviorAnalyzer.js';
import { getBotDetectionService } from './BotDetectionService.js';
import { getScriptedLineDetector } from './ScriptedLineDetector.js';
import {
  trackPixelPlacement,
  trackColorUsed,
  trackCoordinateVisit,
  trackActiveDay,
  trackCanvasVisit,
...
const scriptedLineDetector = getScriptedLineDetector();
...
export default async function drawByOffsets(
  ...
  try {
    ...
    let { isBanned, isProxy, riskScore } = await ip.getAllowance();

    const behaviorResult = behaviorAnalyzer.recordPixelPlacement(
      ipString, user?.id, canvasId,
    );
    if (behaviorResult.score >= 50 || behaviorAnalyzer.isSuspicious(ipString)) {
      isProxy = true;
      logger.warn(
        `Behavior-based proxy detection: ${ipString} | Score: ${behaviorResult.score}`,
      );
    }

    for (let u = 0; u < pixels.length; u += 1) {
      const [offset, color] = pixels[u];
      const [x, y] = getPixelFromChunkOffset(i, j, offset, canvasSize, is3d);
      botDetectionService.recordPixel(
        user?.id || null,
        ipString,
        canvasId,
        x,
    ...
    if (userId && pxlCnt > 0) {
      trackPixelPlacement(userId, canvasId, is3d, pixels[0][1], pxlCnt);
      trackActiveDay(userId);
      trackCanvasVisit(userId, canvasId);

      for (let u = 0; u < pxlCnt; u += 1) {
        const [offset, color] = pixels[u];
        trackColorUsed(userId, color);
        const [x, y] = getPixelFromChunkOffset(i, j, offset, canvasSize, is3d);
        trackCoordinateVisit(userId, x, y);
      }
    }
...
import { awardFirstPixelBadge } from '../data/sql/awardBadge.js';
...
>>>>>>> C:/Users/brioc/.windsurf/worktrees/pixuniverse/pixuniverse-26cf27c6/src/core/draw.js
