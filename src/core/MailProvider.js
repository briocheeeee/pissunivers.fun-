...
export class MailProvider {
  ...
  postVerifyMail(to, name, host, lang, code) {
    ...
    const subject = t`Welcome ${name} to Pixuniverse, please verify your mail`;
    const html = `<em>${t`Hello ${name}`}</em>,<br />
      ...
      ${t`Thanks`}<br /><br />
      <img alt="" src="https://pixunivers.fun/tile.png" style="height:64px; width:64px" />`;
  ...
  postNewLocationMail(to, name, host, lang, ip) {
    ...
    const subject = t`Login from new location on Pixuniverse`;
    const html = `<em>${t`Hello ${name}`}</em>,<br />
${t`Someone logged into your account from a new location, with the IP:`} ${ip}<br />
${t`If this was you, please ignore this email. If it wasn't you, please go to <a href="https://pixunivers.fun">pixunivers.fun</a> and log-out any session that you are unaware of under "Login Methods" in the User Area and change your password.`}<br />
${t`Thanks`}<br /><br />
<img alt="" src="https://pixunivers.fun/tile.png" style="height:64px; width:64px" />`;
    this.sendMail(to, subject, html);
  }
  ...
  postPasswdResetMail(to, ip, host, lang, code) {
    ...
    const subject = t`You forgot your password for Pixuniverse? Get a new one here`;
