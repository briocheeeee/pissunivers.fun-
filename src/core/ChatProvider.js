<<<<<<< C:/Users/brioc/Documents/GitHub/pixuniverse/src/core/ChatProvider.js
/*
...
import { checkChatGreeting } from '../data/sql/DailyObjective.js';
...
export class ChatProvider {
  constructor() {
    ...
    this.substitutes = [
      {
        regexp: /http[s]?:\/\/(old.)?pixunivers\.fun\/#/g,
        replace: '#',
      },
    ];
  ...
  async sendMessage(user, ip, message, channelId, lang, ttag) {
    ...
    try {
      factionInfo = await getUserFactionInfo(id);
    } catch (e) {
      // ignore faction fetch errors
    }

    const userAvatar = user.data.avatar || null;
    let userBadges = user.data.chatBadges || [];
    if (userBadges.length === 0) {
      userBadges = await getChatBadgeSlugs(id);
    }

    checkChatGreeting(id, message);
...
import { getUserFactionInfo } from '../data/sql/FactionMember.js';
import { getChatBadgeSlugs } from '../data/sql/Badge.js';
import { trackChatMessageForBadge } from '../data/sql/awardBadge.js';
import ChatMessageBuffer from './ChatMessageBuffer.js';
import socketEvents from '../socket/socketEvents.js';
import { ban, unban } from './ban.js';
import {
...
export default new ChatProvider();
>>>>>>> C:/Users/brioc/.windsurf/worktrees/pixuniverse/pixuniverse-26cf27c6/src/core/ChatProvider.js
