...
export function renderPlaceholder(
  ...
  const { brushSize } = state.gui;
  const half = Math.floor(brushSize / 2);

  const [hx, hy] = hover;
  const [sx, sy] = worldToScreen(view, scale, $viewport, [hx - half, hy - half]);

  const totalSize = brushSize * scale * PLACEHOLDER_SIZE;
  const centerOffset = (brushSize * scale) / 2;

  viewportCtx.save();
  viewportCtx.translate(sx + centerOffset, sy + centerOffset);
  const angle = Math.sin(Date.now() / 250) / 4;
  viewportCtx.rotate(angle);
  viewportCtx.fillStyle = '#000';
  viewportCtx.fillRect(
    -(totalSize / 2) - PLACEHOLDER_BORDER,
    -(totalSize / 2) - PLACEHOLDER_BORDER,
    totalSize + (2 * PLACEHOLDER_BORDER),
    totalSize + (2 * PLACEHOLDER_BORDER),
  );
...
export function renderPotatoPlaceholder(
  ...
  const [hx, hy] = hover;
  const [sx, sy] = worldToScreen(view, scale, $viewport, [hx - half, hy - half]);
  const totalScale = brushSize * scale;

  viewportCtx.save();
  viewportCtx.fillStyle = '#000';
  viewportCtx.fillRect(sx - 1, sy - 1, 4, totalScale + 2);
  viewportCtx.fillRect(sx - 1, sy - 1, totalScale + 2, 4);
  viewportCtx.fillRect(sx + totalScale - 2, sy - 1, 4, totalScale + 2);
  viewportCtx.fillRect(sx - 1, sy + totalScale - 2, totalScale + 1, 4);
  viewportCtx.fillStyle = palette.colors[selectedColor];
  viewportCtx.fillRect(sx, sy, 2, totalScale);
  viewportCtx.fillRect(sx, sy, totalScale, 2);
  viewportCtx.fillRect(sx + totalScale - 1, sy, 2, totalScale);
  viewportCtx.fillRect(sx, sy + totalScale - 1, totalScale, 2);
  viewportCtx.restore();
}
