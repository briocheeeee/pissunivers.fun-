...
class TemplateLoader {
  ...
  async shareTemplate(title) {
    ...
    const tData = list.find((z) => z.title === title);
    if (!tData) {
      this.#store.dispatch(pAlert(t`Error :(`, t`Template not found`, 'error'));
      return null;
    }
    const fileData = await this.#fileStorage.loadFile(tData.imageId);
    if (!fileData) {
      this.#store.dispatch(pAlert(t`Error :(`, t`Template image not found`, 'error'));
      return null;
    }
    ...
    try {
      ...
      if (!response.ok) {
        this.#store.dispatch(pAlert(t`Error :(`, result.error || t`Failed to share`, 'error'));
        return null;
      }
      this.#store.dispatch(pAlert(t`Success!`, t`Template shared! URL copied to clipboard.`, 'success'));
      const shareUrl = `${window.location.origin}/template/${result.uuid}`;
      navigator.clipboard?.writeText(shareUrl);
      return shareUrl;
    } catch (err) {
      this.#store.dispatch(pAlert(t`Error :(`, err.message, 'error'));
      return null;
    }
  }

  async importFromUrl(uuid) {
    try {
      const response = await fetch(`/api/templates/shared/${uuid}`, {
        credentials: 'include',
      });
      if (!response.ok) {
        const result = await response.json();
        this.#store.dispatch(pAlert(t`Error :(`, result.error || t`Template not found`, 'error'));
        return false;
      }
      const data = await response.json();
      const buffer = await base64ToBuffer(data.imageData);
      const blob = new Blob([buffer], { type: data.mimetype });
      const { list } = this.#store.getState().templates;
      let { title } = data;
      let counter = 1;
      while (list.some((t) => t.title === title)) {
        counter += 1;
        title = `${data.title} (${counter})`;
      }
      await this.addFile(blob, title, data.canvasId, data.x, data.y);
      this.#store.dispatch(pAlert(t`Success!`, t`Template imported!`, 'success'));
      return true;
    } catch (err) {
      this.#store.dispatch(pAlert(t`Error :(`, err.message, 'error'));
      return false;
