...
class SocketEvents extends EventEmitter {
   ...
   * @param faction faction info object { id, tag, name } or null
   */
  ...
  broadcastVoidBoost(endTime) {
    this.emit('voidBoost', endTime);
  }
  ...
  /**
   * send admin message to a specific user
   * @param userId numerical id of user
   * @param message message text
   * @param fromName name of the admin who sent it
   */
  sendAdminMessage(userId, message, fromName) {
    this.emit('adminMessage', userId, message, fromName);
  }
}
