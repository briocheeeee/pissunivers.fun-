...
import { getCooldownFactor, getAllCountryCooldownFactors } from '../core/CooldownModifiers.js';
...
class SocketServer {
  ...
  initialize() {
    ...
    socketEvents.on('adminMessage', (userId, message, fromName) => {
      const text = `am,${JSON.stringify({ message, fromName })}`;
      this.findAllWsByUerId(userId).forEach((ws) => {
        ws.send(text);
      });
    });
    ...
    socketEvents.on('voidBoost', (endTime) => {
      this.broadcast(`vb,${JSON.stringify(endTime)}`);
    });
    ...
    try {
      ...
      while (!client.done) {
        ...
        if (ws.readyState === WebSocket.OPEN && ws.canvasId !== null) {
          ...
          if (!onlineSets[canvasId]) {
            onlineSets[canvasId] = new Set();
          }
          onlineSets[canvasId].add(ipString);
        }
        client = it.next();
      }
      const online = {};
      for (const canvasId of Object.keys(onlineSets)) {
        online[canvasId] = Array.from(onlineSets[canvasId]);
      }
    ...
    try {
      ...
      switch (opcode) {
        ...
        case REG_CANVAS_OP: {
          ...
          const baseCd = canvas.bcd || 4;
          const country = ws.ip?.country;
          const countryFactors = getAllCountryCooldownFactors();
          const countryFactor = country ? (countryFactors[country] || 1) : 1;
          const totalFactor = getCooldownFactor(country, ipString);
          ws.send(`cdm,${JSON.stringify({
            base: baseCd,
            country: countryFactor,
            event: totalFactor / countryFactor,
            factor: totalFactor,
          })}`);
          break;
        }
