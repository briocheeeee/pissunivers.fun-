...
const User = sequelize.define('User', {
  ...
  chatBadges: {
    ...
    defaultValue: null,
    get() {
      const value = this.getDataValue('chatBadges');
      return value ? JSON.parse(value) : [];
    },
    set(value) {
      this.setDataValue('chatBadges', value ? JSON.stringify(value) : null);
    },
  },
});
...
export async function getUserInfosToIds(ids) {
...
export async function setAvatar(id, avatar) {
  try {
    await User.update({ avatar }, { where: { id }, returning: false });
  } catch (error) {
    console.error(`SQL Error on setAvatar: ${error.message}`);
    return false;
  }
  return true;
}
...
export async function getAvatar(id) {
  try {
    ...
    console.error(`SQL Error on getAvatar: ${error.message}`);
    return null;
  }
}
