...
const migrationSequelize = new Sequelize(MYSQL_DATABASE, MYSQL_USER, MYSQL_PW, {
  host: MYSQL_HOST,
  dialect: 'mysql',
  define: {
    timestamps: false,
...
END`,
  ...
  const isMariaDB = (await migrationSequelize.query('SELECT VERSION() AS version'))[0][0].version.includes('MariaDB');
  if (!isMariaDB) {
