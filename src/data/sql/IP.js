...
import { USE_PROXYCHECK, PROXY_RISK_THRESHOLD } from '../../core/config.js';
...
COALESCE(p.isProxy, 0) AS isProxy, COALESCE(p.riskScore, 0) AS riskScore,
w.ip IS NOT NULL AS isWhitelisted,
...
WHERE i.ip = IP_TO_BIN(?)`, {
    ...
    if (ipAllowance) {
      ipAllowance.isProxy = ipAllowance.isProxy === 1 || ipAllowance.riskScore >= PROXY_RISK_THRESHOLD;
      ipAllowance.isWhitelisted = ipAllowance.isWhitelisted === 1;
      ipAllowance.riskScore = ipAllowance.riskScore || 0;
...
ON DUPLICATE KEY UPDATE min = VALUES(min), max = VALUES(max), mask = VALUES(mask), country = VALUES(country), org = VALUES(org), descr = VALUES(descr), asn = VALUES(asn), expires = VALUES(expires)`, {
      ...
      if (pcData) {
        const {
          ...
          score = 0, flags = null, checkCount = 1,
        } = pcData;
        const flagsStr = Array.isArray(flags) ? flags.join(',').slice(0, 500) : (flags || null);
        await sequelize.query(
          `INSERT INTO Proxies (ip, isProxy, type, operator, city, devices, subnetDevices, riskScore, flags, checkCount, expires) VALUES (IP_TO_BIN(?), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON DUPLICATE KEY UPDATE isProxy = VALUES(isProxy), type = VALUES(type), operator = VALUES(operator), city = VALUES(city), devices = VALUES(devices), subnetDevices = VALUES(subnetDevices), riskScore = VALUES(riskScore), flags = VALUES(flags), checkCount = VALUES(checkCount), ip = VALUES(ip), expires = VALUES(expires)`, {
            replacements: [
              ipString,
              isProxy, type, operator, city, devices, subnetDevices,
              score, flagsStr, checkCount,
...
/**
 * get IID of user by user id
 * @param userId user id
 * @return null | uuid as String (first IID found)
 */
export async function getIIDofUserId(userId) {
  try {
    const result = await sequelize.query(
      // eslint-disable-next-line max-len
      'SELECT BIN_TO_UUID(i.uuid) AS \'iid\' FROM UserIPs l INNER JOIN IPs i ON l.ip = i.ip WHERE l.uid = ? LIMIT 1', {
        replacements: [userId],
        raw: true,
        type: QueryTypes.SELECT,
      },
    ...
    console.error(`SQL Error on getIIDofUserId: ${error.message}`);
  }
  return null;
}
