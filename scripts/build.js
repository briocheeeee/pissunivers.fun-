...
function cleanUpBeforeBuild(doBuildServer, doBuildClient) {
  ...
  if (doBuildServer) {
    ...
    [
      ...
      path.join('src', 'core', 'badges.json'),
...
function buildServer() {
  ...
  return compile(config).then(() => {
    process.stdout.write(`\x1b[33mServer build took ${Math.round((Date.now() - ts) / 1000)}s\x1b[0m\n`);
  });
}
...
async function build() {
  ...
  if (doBuildServer) {
    buildPromises.push(buildServer());
  }
  ...
  if (doBuildClient) {
    ...
    buildPromises.push(clientBuild());
  }

  await Promise.all(buildPromises);

  if (doBuildClient) {
    const postBuildPromises = [minifyCss()];
    if (doBuildServer) {
      postBuildPromises.push(createImages());
    }
    await Promise.all(postBuildPromises);
  }
  ...
  if (!development && doBuildClient) {
    ...
    await buildLanguages(avlangs, true, parallel && cpuCount);
    await minifyJs(avlangs, parallel && cpuCount);
  }
